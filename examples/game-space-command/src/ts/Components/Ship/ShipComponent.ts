...
import { ICoordinate, Coordinate } from "../../../../../../src/ts/gamelib/DataTypes/Coordinate";
...
import { IWeapon, CreateWeapon, DisplayWeapon, PullTrigger, RemoveBullet } from "./WeaponComponent";
import { IExhaust, CreateExhaust, DisplayExhaust, ExhaustCopyToUpdated } from "./ThrustComponent";
import { IExplosion, CreateExplosion, DisplayExplosion, UpdateExplosion } from "./ExplosionComponent";
...
export function ShipSounds(ship: IShip): void {
    if (ship.crashed) {
        AsteroidAssets.assets.explosion.playOnce();
    }
    if (ship.exhaust.thrustOn) {
        AsteroidAssets.assets.thrust.play();
    } else {
        AsteroidAssets.assets.thrust.pause();
    }
    if (ship.weapon1.fired) {
        AsteroidAssets.assets.gun.replay();
    }
}
